...
class SuperMarioBrosEnv(NESEnv):
    ...
    def __init__(self, rom_mode='vanilla', lost_levels=False, target=None):
        ...
        # setup a variable to keep track of the last frames y position
        ...
        self._backup()
        # create a list to hold last 100 x_positions
        self._x_history = np.zeros(100)
        # create a counter for the first 100 steps
        self._first_100 = 0
        # X Displacement from one step to the next
        self._x_dis = 0
        # Y Displacment from one step to the next
        self._y_dis = 0
    ...
    def _flag_get(self):
        ...
        if self._is_world_over or self._is_stage_over:
            print("Get Flag!")
    ...
    def add_count(self):
        if self._first_100 < 100:
            self._first_100 = self._first_100 + 1
    @property
    def average(self):
        # Calculates the average x vauel of the last 100 steps
        if self._first_100 == 0:
            return 0
        return(np.sum(self._x_history) / self._first_100 )
    @property
    def update_x_history(self):
        if self._first_100 == 100:
            self._x_history = np.append(self._x_history,self._x_position)
            self._x_history = np.delete(self._x_history,0)
        else:
            self._x_history[self._first_100] = self._x_position
    ...
    def _x_reward(self):
        ...
        #self._x_dis = _reward
        #self._y_dis = self._y_position -self._y_position_last
        self._x_position_last = self._x_position
        self.add_count
        self.update_x_history
    ...
    def _get_reward(self):
        ...
        return self._x_reward + self._time_penalty + self._death_penalty # + self._prox_x # + self._y_reward # + self._stuck_penalty
    ...
    def _get_info(self):
        ...
        return np.array([ self._x_position,self._y_position,self._x_vel,self._y_vel,self._time], dtype=np.float32)  
    """
